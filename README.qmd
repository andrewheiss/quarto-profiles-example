...
With these three files, rendering the document will automatically use the `nice` profile and generate a PDF like this at `output/example.pdf`:

![Nice PDF](img/example-nice.png){width=100%}
...
This will tell Quarto to use the `ugly` profile and it will generate a PDF like this at `output-ugly/example.pdf`:

![Ugly PDF](img/example-ugly.png){width=100%}
